...
import FdvUploadList from './FdvUploadList';
...
export default function Dashboard({ onLogout }: DashboardProps) {
  ...
  const tabs = ['Prosedyre & Instrukser', 'FDV Dokumenter', 'Bygg Oversikt', 'Prosess-skjema', 'KPI'];
  ...
  const navItems = [
    ...
    { id: 'Buildings', icon: Building, label: 'Bygg' },
    { id: 'FDV', icon: Upload, label: 'FDV' },
    { id: 'Docs', icon: FileText, label: 'Docs' },
    { id: 'Flows', icon: GitBranch, label: 'Flyt' },
      ...
      <div className="flex max-w-7xl mx-auto">
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      // Update active tab based on nav item
                      if (item.id === 'Buildings') setActiveTab('Bygg Oversikt');
                      else if (item.id === 'FDV') setActiveTab('FDV Dokumenter');
                      else if (item.id === 'Docs') setActiveTab('Prosedyre & Instrukser');
                      else if (item.id === 'Flows') setActiveTab('Prosess-skjema');
                      else if (item.id === 'KPI') setActiveTab('KPI');
                      else if (item.id === 'Dashboard') setActiveTab('Prosedyre & Instrukser');
                    }}
        ...
        <main className="flex-1 p-8">
          <div className="space-y-8">
            ...
            {/* FDV Documents Tab */}
            {activeTab === 'FDV Dokumenter' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className="font-semibold text-foreground text-2xl">FDV Dokumenter</h2>
                  <p className="text-muted-foreground">Forvaltning, Drift og Vedlikehold dokumenter</p>
                </div>
                
                <FdvUploadList buildingId="40" />
              </div>
            )}

            {/* Buildings Overview Tab */}
            {activeTab === 'Bygg Oversikt' && (
              <div className="bg-card border border-border rounded-lg shadow-card overflow-hidden">
                <div className="px-6 py-4 border-b border-border">
                  <h2 className="font-semibold text-foreground">Bygg Oversikt</h2>
                </div>
                <div className="p-6">
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      <p className="text-muted-foreground">Oversikt over alle bygg (40-130)</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                      ...
                      {Array.from({ length: 12 }, (_, i) => {
                        const buildingNumber = 40 + i;
                        return (
                          <div key={buildingNumber} className="bg-card border border-border rounded-lg p-4 hover:shadow-elevated transition-shadow">
                            <div className="flex items-center gap-3 mb-3">
                              <Building className="w-6 h-6 text-primary" />
                              <div className="flex-1">
                                <h4 className="font-bold text-foreground">Bygg {buildingNumber}</h4>
                                <p className="text-muted-foreground text-xs">Hovedbygg</p>
                              </div>
                            </div>
                            <div className="space-y-2 text-xs">
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">Status:</span>
                                <span className="text-primary font-medium">Aktiv</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-muted-foreground">FDV:</span>
                                <span className="text-foreground">{Math.floor(Math.random() * 5) + 1} dok.</span>
                              </div>
                            </div>
                            <button className="w-full mt-3 bg-secondary text-secondary-foreground px-3 py-2 rounded-md text-xs font-medium hover:opacity-90 transition-colors">
                              Vis detaljer
                            </button>
                          </div>
                        );
